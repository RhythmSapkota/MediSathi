...
const DoctorSession = () => {
  ...
  const handleHospitalSelect = (event) => {
    ...
    const selectedHospitalInfo = hospitals.find(
  ...
  const handleStartSession = () => {
    ...
    const usersArr = todaysBooking.map((booking) => {
      const foundUser = bookedByUsers.find(
        (user) => booking.bookedBy === user._id
      );
      const foundHospital = hospitals.find(
        (hospital) => booking.hospitalId === hospital._id
      );
    ...
    navigate(
      `/dashboard/daily-sessions/appointmentreport?usersArr=${encodeURIComponent(
        usersArrJSON
      )}`
    );
        ...
        {showDetails && selectedHospital && sessionStartTime != null && (
          <div>
            <StartButton onClick={handleStartSession}>
              <FaPlay />
              {!ready
                ? `Session Starts in ${calculateTimeRemaining(
                    sessionStartTime
                  )}`
                : "Start Session"}
            </StartButton>
