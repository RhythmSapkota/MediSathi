...
import Hospital from '../modals/hospitalModal.js'; // Assuming the path to your Hospital model is correct
...
export const registerDoctor = async (req, res) => {
  console.log(req.user.role);
   ...
   if (req.user.role === "admin"){
    const doctors = req.body; // Assuming that req.body is an array of doctors
    const doctorIds = [];
...
let password = crypto.randomBytes(6).toString('hex');
  ...
  const templateData = {
    ...
    res.status(StatusCodes.CREATED).json({ msg: "Doctors Registered", doctorIds });
